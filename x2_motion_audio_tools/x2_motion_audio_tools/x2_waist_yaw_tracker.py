#!/usr/bin/env python3
"""Track the stereo target point with the X2 waist yaw joint only."""
...
class X2WaistYawTracker(Node):
    """Turn waist_yaw_joint toward a PointStamped target in the camera frame."""

    def __init__(self) -> None:
        super().__init__("x2_waist_yaw_tracker")
        ...
        self.declare_parameter("yaw_gain", 0.45)
        self.declare_parameter("start_threshold_deg", 8.0)
        ...
        self.declare_parameter("soft_limit_deg", 35.0)
        ...
        self.start_threshold_rad = math.radians(
            float(self.get_parameter("start_threshold_deg").value)
        )
        ...
        self.head_yaw_position: Optional[float] = None
        ...
        self.get_logger().info(
            "Waist yaw tracker started: "
            f"target={self.target_topic}, head_state={self.head_state_topic}, "
            f"waist_state={self.waist_state_topic}, "
            f"command={self.waist_command_topic}, enabled={self.enabled}, "
    ...
    def target_callback(self, msg: PointStamped) -> None:
        if msg.point.z < self.min_depth_m:
            return
        bearing_rad = self.torso_bearing_from_camera_bearing(
            math.atan2(float(msg.point.x), float(msg.point.z))
        )
        ...
        if start_positions is not None:
            self.target_yaw_position = self.desired_yaw_for_bearing(
                bearing_rad, start_positions[WAIST_YAW_INDEX]
            )

    def head_state_callback(self, msg: JointStateArray) -> None:
        for joint in msg.joints:
            if getattr(joint, "name", "") == HEAD_YAW_JOINT:
                self.head_yaw_position = float(joint.position)
                return
        if msg.joints:
            self.head_yaw_position = float(msg.joints[0].position)

    def torso_bearing_from_camera_bearing(self, camera_bearing_rad: float) -> float:
        if self.head_yaw_position is None:
            return camera_bearing_rad

        # target_point x is positive image-right. X2 head_yaw_joint is positive
        # left, so subtracting head yaw gives a right-positive torso bearing.
        return camera_bearing_rad - self.head_yaw_position
    ...
    def initialize_ruckig(self) -> None:
        ...
        self.ruckig_input.target_position = list(self.waist_positions)
        ...
        self.get_logger().info("Ruckig waist tracker initialized from joint state.")
    ...
    def control_loop(self) -> None:
        ...
        if target_is_stale or self.last_target_bearing_rad is None:
            ...
            if self.hold_on_lost:
                self.publish_waist_command(
                    self.command_positions or self.waist_positions,
                    [0.0] * len(WAIST_JOINTS),
                )
            return
        ...
        if self.target_yaw_position is None:
            self.target_yaw_position = self.desired_yaw_for_bearing(
                ...
                start_positions[WAIST_YAW_INDEX],
            )

        target_positions = list(start_positions)
        target_positions[WAIST_YAW_INDEX] = self.target_yaw_position

        if self.use_ruckig:
            next_positions, velocities = self.next_ruckig_step(target_positions)
        else:
            next_positions, velocities = self.next_waist_step(target_positions)
    ...
    def desired_yaw_for_bearing(self, bearing_rad: float, current_yaw: float) -> float:
        ...
        if abs(bearing_rad) < self.start_threshold_rad:
            return clamp(current_yaw, self.yaw_lower_limit, self.yaw_upper_limit)
        if abs(bearing_rad) < self.center_deadzone_rad:
            return clamp(current_yaw, self.yaw_lower_limit, self.yaw_upper_limit)

        # Bearing is positive when the target is to the robot's right; positive
        # waist yaw is left, matching the head tracker sign convention.
        return clamp(
    ...
    def next_ruckig_step(
        ...
        if result not in [ruckig.Result.Working, ruckig.Result.Finished]:
            ...
            if now - self.last_ruckig_warning > 3.0:
                ...
                self.get_logger().warn(
                    "Ruckig waist tracker step failed; using velocity-limited fallback."
                )
            return self.next_waist_step(target_positions)
    ...
    def next_waist_step(
        ...
        positions[WAIST_YAW_INDEX] = move_toward(
            start_positions[WAIST_YAW_INDEX],
            target_positions[WAIST_YAW_INDEX],
            max_delta,
        )
        positions[1] = start_positions[1]
        positions[2] = start_positions[2]
    ...
    def ruckig_max_velocity(self) -> list[float]:
        return [self.max_yaw_velocity] * len(WAIST_JOINTS)

    def ruckig_max_acceleration(self) -> list[float]:
        return [self.max_yaw_acceleration] * len(WAIST_JOINTS)

    def ruckig_max_jerk(self) -> list[float]:
        return [self.max_yaw_jerk] * len(WAIST_JOINTS)
    ...
    def publish_waist_command(
        ...
        if self.dry_run:
            ...
            if now - self.last_dry_run_log > 1.0:
                ...
                self.get_logger().info(
                    "dry_run waist command: "
                    f"yaw={positions[WAIST_YAW_INDEX]:+.3f}rad, "
